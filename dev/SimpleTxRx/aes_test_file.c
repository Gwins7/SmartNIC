...
uint8_t ascii_to_hex(char *c){
	uint8_t a1=(c[0]>='a'?c[0]-'a'+10:c[0]-'0');
	uint8_t a2=(c[1]>='a'?c[1]-'a'+10:c[1]-'0');
	return (uint8_t)((a1 << 4) + a2);
}
...
int main(int argc,char **argv) {
	...
	*/
	...
	uint8_t a,b,c;
	printf("key = ");
	for (a=0;a<sizeof(key);a++) printf("%02x",key[a]);
	printf("\n");
	aes_key_expansion(key, w);

	FILE *fd1 = fopen("aes_cipher_input","r");
	FILE *fd2 = fopen("aes_cipher_output","w");
	char input[33];
	uint8_t in[16];
	uint8_t out[16];
	...
	while (fread(input,33,1,fd1)!=0){
		...
		for (a=0;a<16;a++) in[a] = ascii_to_hex(input + 2*a);
		
		if (cipher_mode) aes_cipher(in /* in */, out /* out */, w /* expanded key */);
		else aes_inv_cipher(in, out, w);

		for (a=0;a<16;a++) fprintf(fd2,"%02x",out[a]);
		fprintf(fd2,"\n");
	}

	fflush(fd2);
	printf("aes cipher mode %d complete!\n",cipher_mode);
